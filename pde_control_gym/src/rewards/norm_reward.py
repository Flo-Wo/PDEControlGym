...
class NormReward(BaseReward):
    ...
    def __init__(
        ...
        nt: int = None,
        norm: str = "2",
        horizon: str = "temporal",
        truncate_penalty: float = -1e-4,
        terminate_reward: float = 1e2,
        t_horizon_length: int = 5,
        *extras,
    ):
        if nt is None:
            raise Exception(
                "Number of simulation steps must be specified in the NormReward class."
            )
        self.nt = nt
        self.norm = norm
    ...
    def reward(
        ...
        if uVec == None:
            raise Exception(
                "Class NormReward attempted to call reward function and recieved a None vector to compute on"
            )
        if time_index == None:
            raise Exception(
                "Class NormReward attempted to call reward fucntion and recieved a None time_index parameter to identify the reward step"
            )
                ...
                if time_index > self.t_horizon_length:
                    for i in range(0, self.t_horizon_length):
                        result += np.linalg.norm(
                            uVec[time_index - 1 * i], ord=self.norm
                        )
