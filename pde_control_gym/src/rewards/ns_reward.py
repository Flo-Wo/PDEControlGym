...
class NSReward(BaseReward):
    ...
    def reward(
        ...
        time_index: int = None,
        U_ref: Union[float, np.ndarray] = None,
        action: Union[float, np.ndarray] = None,
        action_ref: Union[float, np.ndarray] = None,
    ):
        """
        reward
        ...
        return (
            -1
            / 2
            * np.linalg.norm(uVec[time_index] - U_ref[time_index]) ** 2
            / uVec.shape[1]
            / uVec.shape[2]
            - self.gamma / 2 * np.linalg.norm(action - action_ref[time_index]) ** 2
        )

    def reward_uVec_without_time(
        self,
        uVec: np.ndarray = None,
        time_index: int = None,
        U_ref: Union[float, np.ndarray] = None,
        ...
        return (
            -1
            / 2
            * np.linalg.norm(uVec - U_ref[time_index]) ** 2
            / uVec.shape[1]
            / uVec.shape[2]
            - self.gamma / 2 * np.linalg.norm(action - action_ref[time_index]) ** 2
        )
