...
class NavierStokes2D(PDEEnv2D):
    ...
    def apply_boundary(
        self, u: np.ndarray, v: np.ndarray, action: Union[float, np.ndarray]
    ):
        """
            ...
            for i in range(2):
                ...
                match condition:
                    case "Neumann":
                        ...
                        if i == 0:
                            u[xidx, yidx] = u[xidx2, yidx2]
                        else:
                            v[xidx, yidx] = v[xidx2, yidx2]
                    case "Dirchilet":
                        if i == 0:
                            u[xidx, yidx] = 0
                        else:
    ...
    def step(self, action: Union[float, np.ndarray]):
        ...
        # predictor step
        u_pred = u_prev + dt * (
            -u_prev * dudx - v_prev * dudy + self.KINEMATIC_VISCOSITY * laplace_u_prev
        )
        v_pred = v_prev + dt * (
            -u_prev * dvdx - v_prev * dvdy + self.KINEMATIC_VISCOSITY * laplace_v_prev
        )
        # apply boundary conditions
        u_pred, v_pred = self.apply_boundary(u_pred, v_pred, action)
        # solve for pressure
        pressure = self.solve_pressure(u_pred, v_pred, p_prev)
        dpdx, dpdy = central_difference(pressure, "x", dx), central_difference(
            pressure, "y", dy
        )
        ...
        reward = self.reward_class.reward(
            self.U, self.time_index, self.U_ref, action, self.action_ref
        )
