...
from pde_control_gym.src.environments2d.navier_stokes2D import (
    central_difference,
    laplace,
)
import time
...
for experiment_i in range(1):
    ...
    for ite in range(1):
        ...
        for t in tqdm(range(T - 1)):
            ...
            dl1dx, dl1dy = central_difference(lam1, "x", dx), central_difference(
                lam1, "y", dy
            )
            dl2dx, dl2dy = central_difference(lam2, "x", dx), central_difference(
                lam2, "y", dy
            )
        ...
        plt.show()
        np.savez(
            "result/NS_optmization.npz",
            U=env.U[:, :, :, 0],
            V=env.U[:, :, :, 1],
            desired_U=np.array(u_target),
            desired_V=np.array(v_target),
            actions=actions,
        )
